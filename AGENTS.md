...
### viewlogs.nu (`bin/viewlogs.nu`)

A Nushell script to manage and view logs generated by other tools.

**Usage:**
```
nu bin/viewlogs.nu <command> [options]
```

**Commands:**
- `clean` - Remove all log files except the most recent one
- `show` - Display log contents (interactive and timestamp-based selection)
