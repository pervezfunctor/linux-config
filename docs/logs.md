...
```bash
~/.local/share/linux-config/nu/logs.nu show           # view the most recent log
~/.local/share/linux-config/nu/logs.nu show --select  # pick a specific timestamp interactively
~/.local/share/linux-config/nu/logs.nu clean          # keep only the latest log
```
